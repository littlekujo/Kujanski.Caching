...
using Kujanski.Caching.SwapCache;
...
namespace Kujanski.Caching.Tests
{
    ...
    public class SwapCacheTests
    {
        ...
        public void IsInitiallyPopulated()
        {
            ...
            Assert.IsNotNull(_sut.ActiveCache);
            Assert.IsTrue(_sut.ActiveCache.Count > 0);
        ...
        {
            _sut.ExecutePopulateFunction();
            var first1 = _sut.ActiveCache[0];
            var second1 = _sut.ActiveCache[1];
            Assert.AreEqual(_sut.PopulationCount, 1);

            _sut.ExecutePopulateFunction();
            var first2 = _sut.ActiveCache[0];
            var second2 = _sut.ActiveCache[1];
        ...
        [TestMethod]
        public void EachCacheContainsDifferentPopulations()
        {
            _sut.ExecutePopulateFunction();
            _sut.ExecutePopulateFunction();
            var first1 = _sut.ActiveCache[0];
